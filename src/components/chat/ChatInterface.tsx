      ...
      try {
        ...
        if ((err as Error).name === 'AbortError') {
          setMessages((prev) =>
            prev.map((m) =>
              m.id === aiMsgId
                ? { ...m, isStreaming: false, content: m.content || '_(stopped)_' }
                : m
            )
          );
          return;
  ...
  const handleContextFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      toast({
        ...
        description: `"${data.fileName}" attached to this conversation.`,
      });
    } catch (err) {
    ...
    <div className="flex flex-col h-full bg-slate-50 dark:bg-slate-950">
      ...
      <div className="flex items-center justify-between px-4 py-2.5 border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shrink-0 gap-3">
        ...
        {!isEmpty && (
          <button
            onClick={clearChat}
            className="flex items-center gap-1.5 text-xs text-slate-400 hover:text-red-500 transition-colors py-1 px-2 rounded-lg hover:bg-red-50 dark:hover:bg-red-950/20 shrink-0"
      ...
      {showScrollBtn && !isEmpty && (
        <div className="absolute bottom-[88px] left-1/2 -translate-x-1/2 z-10">
          <button
            ...
            className="flex items-center gap-1.5 text-xs bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shadow-lg shadow-slate-200/60 dark:shadow-slate-900/60 rounded-full px-3.5 py-1.5 hover:bg-indigo-50 dark:hover:bg-indigo-950/40 hover:border-indigo-300 dark:hover:border-indigo-700 hover:text-indigo-700 dark:hover:text-indigo-300 transition-all duration-200"
      ...
      <div className="shrink-0 border-t border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 px-4 py-3">
        <div className="max-w-3xl mx-auto">
          ...
          <div
            className={cn(
              'flex items-end gap-2 border border-slate-200 dark:border-slate-700 rounded-2xl bg-slate-50 dark:bg-slate-800 px-3 py-2',
          ...
          >
            ...
            <textarea
              ...
              className={cn(
                ...
                'text-slate-900 dark:text-slate-100 placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:outline-none',
          ...
          <p className="text-center text-[11px] text-slate-400 dark:text-slate-500 mt-2 select-none">
            <kbd className="font-mono bg-slate-100 dark:bg-slate-700 border border-slate-200 dark:border-slate-600 px-1.5 py-0.5 rounded text-[10px]">Enter</kbd>{' '}
            send &bull;{' '}
            <kbd className="font-mono bg-slate-100 dark:bg-slate-700 border border-slate-200 dark:border-slate-600 px-1.5 py-0.5 rounded text-[10px]">Shift+Enter</kbd>{' '}
            new line &bull; Not medical advice — always consult a doctor
          </p>
        </div>
      </div>
    </div>
